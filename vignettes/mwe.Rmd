...
## gene level analysis by mirage

### Summary statistics for each rare variant as input. 
...
##  variant level analysis by mirage_vs 



### Input data for `mirage_vs`

`mirage_vs` is the variant set version of `mirage`. The input data structure is the same but with no gene level information. 

```{r}
head(mirage_vs_toy)
```

### Running `mirage_vs` on toy data

```{r, cache=T}
res_vs=mirage_vs(mirage_vs_toy,n1=4315,n2=4315)
```
The output is a list of parameter estimate and posterior probability 

- `eta.est`: Estimate of eta and p value 

```{r}
res_vs$eta.est
```

- `post.prob`: BF and posterior probability of all variants 

```{r}
head(res_vs$post.prob)
```


### Run `mirage_vs` with fixed eta


```{r}
res_vs_fixed_eta=mirage_vs(mirage_vs_toy,n1=4315,n2=4315, estimate.eta = F, fixed.eta = 0.1)
head(res_vs_fixed_eta$post.prob)
```
